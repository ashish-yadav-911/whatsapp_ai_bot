...
#     LOG_LEVEL = os.environ.get('LOG_LEVEL') or 'INFO'  # Default to INFO if not set 
#     LOG_FILE_PATH = os.environ.get('LOG_FILE_PATH') or 'logs/app.log'  # Default log file path
#     LOG_FILE_ROTATION = os.environ.get('LOG_FILE_ROTATION') or 'midnight'  # Default to midnight rotation
#     LOG_BACKUP_COUNT = int(os.environ.get('LOG_BACKUP_COUNT', 7))  # Default to 7 backups if not set
#     ENABLE_ERROR_EMAIL = os.environ.get('ENABLE_ERROR_EMAIL', 'False').lower() == 'true'  # Convert to boolean
#     LOG_EMAIL_HOST = os.environ.get('LOG_EMAIL_HOST')
#     LOG_EMAIL_PORT = int(os.environ.get('LOG_EMAIL_PORT', 587))  # Default to 587 if not set
#     LOG_EMAIL_USERNAME = os.environ.get('LOG_EMAIL_USERNAME')
#     LOG_EMAIL_PASSWORD = os.environ.get('LOG_EMAIL_PASSWORD')
#     LOG_EMAIL_RECIPIENTS = os.environ.get('LOG_EMAIL_RECIPIENTS', '').split(',') if os.environ.get('LOG_EMAIL_RECIPIENTS') else []
#     LOG_RETENTION_DAYS = int(os.environ.get('LOG_RETENTION_DAYS', 30))  # Default to 30 days if not set
...
#     CRM_API_BASE_URL = os.environ.get('CRM_API_URL')
#     CRM_API_KEY = os.environ.get('CRM_API_KEY')
#     DOCTOR_ID_STR = os.environ.get('DOCTOR_ID') # Read as string initially
#     CLINIC_ID_STR = os.environ.get('CLINIC_ID') # Read as string initially

#     # New IDs from curl, read as strings
#     PROCEDURE_ID_STR = os.environ.get('PROCEDURE_ID')
#     HEALTH_OPERATOR_ID_STR = os.environ.get('HEALTH_OPERATOR_ID')


#     # Chat Transfer Config
#     WHATSAPP_TRANSFER_NUMBER = os.environ.get('WHATSAPP_TRANSFER_NUMBER')

#     # Attributes to hold converted integer IDs
#     DOCTOR_ID = None
#     CLINIC_ID = None
#     PROCEDURE_ID = None
#     HEALTH_OPERATOR_ID = None
...
#          if not Config.CRM_API_BASE_URL or not Config.CRM_API_KEY or Config.DOCTOR_ID is None or Config.CLINIC_ID is None or Config.PROCEDURE_ID is None or Config.HEALTH_OPERATOR_ID is None:
#               print("Warning: CRM/Scheduling API details (URL, Key, Doctor/Clinic/Procedure/Operator IDs) not fully set or are invalid. Meeting scheduling will not work.")
